 ...
 */
class Symfony2_Sniffs_Commenting_FunctionCommentSniff implements PHP_CodeSniffer_Sniff
{
        ...
        if (empty($params) === false) {
            ...
            foreach ($params as $param) {
                ...
                if ($previousParam !== null) {
                    ...
                    if ($this->alignsCommentWith($param, $previousParam) === false) {
     ...
     * @param PHP_CodeSniffer_CommentParser_ParameterElement $current The current param
     *                                                                to check
     *                                                                alignment with.
     *
    ...
    public function alignsCommentWith(
        PHP_CodeSniffer_CommentParser_ParameterElement $current,
        PHP_CodeSniffer_CommentParser_ParameterElement $other
    ) {
        // Compares the index before param comment.
        $wsOther      = strlen($other->getWhiteSpaceBeforeComment());
        $otherComment = strlen($other->getVarName()) + $wsOther;
        $wsThis       = strlen($current->getWhiteSpaceBeforeComment());
        $thisComment  = strlen($current->getVarName()) + $wsThis;

        if ($wsOther && $wsThis && $otherComment !== $thisComment) {
            return false;
        }
